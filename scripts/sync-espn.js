// Sync ESPN live scores → Supabase live_fixtures (for in-play data)
// Runs every 5 min via GitHub Actions — ESPN is free, no key needed
...
const v = (o, seed) => +(Math.max(1.01, o * (1 + Math.sin(seed + Date.now() / 30000) * 0.02))).toFixed(2);
...
function mkts(sp, h, a, od, hR, aR) {
  ...
  if (!hO || hO <= 1) {
    const hS = Math.max(.3, 1.3 - hR * .04), aS = Math.max(.3, 1.3 - aR * .04), t = hS + aS + .7;
    hO = v(M / (hS / t), sd); dO = v(M / (.7 / t), sd + 1); aO = v(M / (aS / t), sd + 2);
  }
  if (!dO || dO <= 1) dO = v(3.3, sd + 5);
  hO = v(hO, sd + 10); aO = v(aO, sd + 12); dO = v(dO, sd + 11);
  const hp = 1 / hO, dp = 1 / dO, ap = 1 / aO;
  const gp = .42 + Math.min(hp, ap) * .4;
  const gg = .3 + Math.min(hp, ap) / Math.max(hp, ap) * .25;

  if (sp === "soccer") return {
    "1X2": { "1": hO, "X": dO, "2": aO },
    "O/U 2.5": { Over: v(M / gp, sd + 20), Under: v(M / (1 - gp), sd + 21) },
    "DC": { "1X": v(M / (hp + dp), sd + 30), "12": v(M / (hp + ap), sd + 31), "X2": v(M / (dp + ap), sd + 32) },
    "BTTS": { GG: v(M / gg, sd + 40), NG: v(M / (1 - gg), sd + 41) },
  };
...
async function supaUpsert(rows) {
  ...
  const r = await fetch(`${SB_URL}/rest/v1/live_fixtures?on_conflict=id`, {
    method: "POST", headers: SB, body: JSON.stringify(rows),
  });
  ...
  try {
    ...
    for (const e of d.events) {
      ...
      rows.push({
        ...
        status: fin ? "finished" : live ? "live" : "upcoming",
        home_score: hS,
        away_score: aS,
